...
#         self.get_mean()
    ...
    def get_mean(self):
        for y in range(10):
            ...
            print('estimate p(x|y = ', y, ')')
            t1 = time.time()
            a = len(self.x_train[self.y_train == y])
            for i in range(784):
                b = self.x_train[self.y_train == y][:, i]
                self.px_y[y, i] = (len(b[b > 10]) + 1) / (a + 2)
...
# save the p_xy data as csv file
raw_data = BN.px_y.T
labels = ['y=0', 'y=1', 'y=2', 'y=3', 'y=4', 'y=5', 'y=6', 'y=7', 'y=8', 'y=9']
df = pd.DataFrame(raw_data, columns = labels)
df.to_csv('prob1_csv.csv')
