...
class BayesN():
    ...
    def __init__(self):
        ...
        self.smooth = 0.1 # for singularity of sigma
        self.means = []
        self.covs = []
        self.py = np.zeros(10)
        self.Py()
    ...
    def likelihood_log(self, x, y):
        ...
        for i in range(784):
            ...
            prob += -0.5 * np.log(self.covs[y][i,i] + self.smooth) - 0.5 * (x[i] - self.means[y][i]) ** 2 / (self.covs[y][i,i] + self.smooth)
        return prob

    # def test(self):
    #     print(self.likelihood_log(self.x_train[0], 0))
...
#             a = y[i] == self.predict(x[i])
            b = time.time()
            a = self.predict(x[i])
            print(i, 'the predict is ', y[i], a, 'time ', time.time() - b, i / len(x), "%")
...
def main():
    ...
    print("THe time to get mean & cov is: ", c - b)
